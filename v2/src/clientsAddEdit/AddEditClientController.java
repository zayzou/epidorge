 ...
 * @author sof
 */
public class AddEditClientController extends BaseController implements Initializable {
    ...
    private void ajouter(ActionEvent event) {
        ...
        if (!empty) {
            ...
            if (ok) {
                editFour = new ListFour(raisonSocial.getText(), adr.getText(), tel.getText(), commentaire.getText(), Double.valueOf(solde.getText()), idClient);
                try {

                    int i = isSet ? DatabaseUtil.updateClient(editFour) : DatabaseUtil.addClient(editFour);
                    if (i > 0) {
