...
public class UpdateQteController extends BaseController implements Initializable {
    ...
    private void ajouter(ActionEvent event) {
        try {
            boolean empty = ValidateTF.isEmpty(tfQte, labQte, "le champ quantité ne peut pas être vide !");
            if (!empty) {
                boolean ok = AlertMaker.showDialog("MODIFIER", "Voulez vous vraiment modifier " + editVente.getDes() + " ?");
                if (ok) {
                    ...
                    String req = "UPDATE sortieproduit SET qte = ?,total = ? WHERE id = ?";
                    if (qteSortie < qteActuel) {
                        AlertMaker.showMaterialError(rootStackpane, rootAnchorpane,
                                new ArrayList<>(),
                                "Erreur", "La valeur saisie (" + qteActuel + ") est superieur à la quantité sortie ( " + qteSortie + " )"
                        );
    ...
    private int getQteSortie(String des) {
        ...
        try {
            ...
            statement = con.prepareStatement("SELECT qte FROM lignesortie WHERE des_prod = ? "
                    + "AND date = CURRENT_DATE AND lignesortie.employe = (SELECT employes.id_employe"
