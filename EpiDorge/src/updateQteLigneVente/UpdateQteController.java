...
public class UpdateQteController extends BaseController implements Initializable {
        ...
        try {
            ...
            if (!empty) {
                ...
                if (ok) {
                    ...
                    int qteEnStock = getQteEnStock(des);
                    ...
                    if (qteEnStock < qteActuel) {
                        AlertMaker.showMaterialError(rootStackpane, rootAnchorpane,
                                new ArrayList<>(),
                                "Erreur", "La valeur saisie (" + qteActuel + ") est supérieur à la quantité en stock ( " + qteEnStock + " )"
                        );
    ...
    private int getQteSortie(String des) {
        ...
        private int getQteEnStock(String des) {
        int qte = -1;
        try {

            statement = con.prepareStatement("SELECT qte_stock FROM produits WHERE des_prod = ?");
            statement.setString(1, des);
            resultSet = statement.executeQuery();
            if (resultSet.next()) {
                qte = resultSet.getInt("qte_stock");
            }
        } catch (SQLException ex) {
            System.err.println(ex.getMessage());
